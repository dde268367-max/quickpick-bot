...
function registerButtons(bot) {
  bot.on('callback_query', async (query) => {
    ...
    // ─── Навігація ────────────────────────────────────────────────────────────
      ...
      // Зберігаємо локацію, скидаємо тільки кухню/бюджет
      ...
      // Зберігаємо район, скидаємо кухню/бюджет
      const { lat, lng, districtName, isManualDistrict } = user.session;
      user.lastRecs = [];
      if (lat && lng) {
        user.session = { lat, lng, districtName, isManualDistrict };
    ...
    // ─── Вибір страви ─────────────────────────────────────────────────────────
      ...
      track(chatId, 'dish_picked', {
        dish: rec.dish, place: rec.place, price: rec.price,
        cuisine: user.session?.cuisine, is_pro: user.isPro, is_gem: rec.isGem || false,
      });
    ...
    // ─── Профіль ──────────────────────────────────────────────────────────────
      ...
      profileText += `🔍 Пошуків: ${user.searchCount || 0}  •  ❤️ Збережено: ${user.saved.length}\n`;
      ...
      await bot.sendMessage(chatId, `😊 Добре! Повернемось пізніше.`,
